...
function getLocalRepoStatus(
  ...
  try {
    git(expanded, "rev-parse --git-dir");
  ...
  try {
    status.branch = git(expanded, "rev-parse --abbrev-ref HEAD");
  } catch {
  ...
  try {
    execSync(`git -C "${expanded}" fetch --quiet 2>/dev/null`, {
      timeout: 4000,
    });
  } catch {
    /* offline */
  ...
  try {
    ...
    status.lastCommitMessage = "No commits";
  }

  try {
    const log = git(expanded, "log -5 --format=%s___%cr___%ae --no-merges");
    status.recentCommits = log
      .split("\n")
      .filter(Boolean)
      .map((line) => {
...
function getRemoteRepoStatus(
  ...
  try {
    ...
    for (const line of raw.split("\n")) {
      ...
      if (inLog && line.trim()) {
        const [msg, date, email] = line.split("___");
        status.recentCommits.push(
          `${msg} (${date}, ${parseAuthor(email || "")})`,
        );
        continue;
      }
      if (line.startsWith("BRANCH:")) {
        status.isGitRepo = true;
        status.branch = line.slice(7);
      } else if (line.startsWith("DIRTY:")) {
        const n = parseInt(line.slice(6), 10) || 0;
        status.dirtyCount = n;
        status.isDirty = n > 0;
      } else if (line.startsWith("AHEAD:")) {
        status.ahead = parseInt(line.slice(6), 10) || 0;
      } else if (line.startsWith("BEHIND:")) {
        status.behind = parseInt(line.slice(7), 10) || 0;
      } else if (line.startsWith("MSG:")) {
        status.lastCommitMessage = line.slice(4);
      } else if (line.startsWith("DATE:")) {
        status.lastCommitDate = line.slice(5);
      } else if (line.startsWith("EMAIL:")) {
        status.lastCommitAuthor = parseAuthor(line.slice(6));
      } else if (line.startsWith("STATUS:")) {
        status.fullStatus = line.slice(7);
      }
    }
  } catch (e) {
    const err = e as Error;
    status.error = err.message.includes("timed out")
      ? "SSH timeout"
      : "SSH failed";
  }
...
function compareMachines(
  ...
  remote: RepoStatus,
  localName: string,
): { icon: Icon; color: Color; text: string } {
  if (remote.error)
    return {
      icon: Icon.Wifi,
      color: Color.Red,
      text: `${OTHER_MACHINE}: ${remote.error}`,
    };
  if (local.error)
    return {
      icon: Icon.XMarkCircle,
      color: Color.Red,
      text: `${localName}: ${local.error}`,
    };
  ...
  if (!sameBranch) {
    return {
      ...
      color: Color.Orange,
      text: `${localName}: ${local.branch} — ${OTHER_MACHINE}: ${remote.branch}`,
    };
  }
  if (local.isDirty || remote.isDirty) {
    ...
    return {
      icon: Icon.Pencil,
      color: Color.Orange,
      text: `Uncommitted on ${dirtyOn.join(" & ")}`,
    };
  }
  return {
    icon: Icon.ExclamationMark,
...
function detailMarkdown(
  ...
  if (local.ahead > 0 || local.behind > 0)
    md += `- **Remote:** ${local.ahead} ahead, ${local.behind} behind\n`;
  ...
  if (remote) {
    ...
    if (remote.error) {
      ...
      if (remote.ahead > 0 || remote.behind > 0)
        md += `- **Remote:** ${remote.ahead} ahead, ${remote.behind} behind\n`;
...
export default function MachineSyncDashboard() {
  ...
  async function loadData() {
    setIsLoading(true);
    try {
      const locals = WATCHED_REPOS.map((r) =>
        getLocalRepoStatus(r.path, r.label, hostname),
      );
      setLocalRepos(locals);

      try {
        execSync(
          `ssh -o ConnectTimeout=5 -o BatchMode=yes ${OTHER_MACHINE} echo ok`,
          { encoding: "utf8", timeout: 8000 },
        );
        setRemoteReachable(true);
        const remotes = WATCHED_REPOS.map((r) =>
          getRemoteRepoStatus(OTHER_MACHINE, r.path, r.label),
        );
  ...
  function handleGitAction(path: string, action: "push" | "pull") {
    const expanded = expandPath(path);
    try {
      execSync(`git -C "${expanded}" ${action}`, {
        encoding: "utf8",
        timeout: 15000,
      });
      showToast({
  ...
  function handleRemoteGitAction(path: string, action: "push" | "pull") {
    try {
      execSync(
        `ssh -o ConnectTimeout=5 -o BatchMode=yes ${OTHER_MACHINE} "cd ${path} && git ${action}"`,
        { encoding: "utf8", timeout: 15000 },
      );
      showToast({
        style: Toast.Style.Success,
        title: `${OTHER_MACHINE}: git ${action} complete`,
      });
      loadData();
    } catch (e) {
      const err = e as Error;
      showToast({
        style: Toast.Style.Failure,
        title: `${OTHER_MACHINE}: git ${action} failed`,
        message: err.message,
      });
    }
  }
  ...
  function quickSync(localPath: string, remotePath: string) {
    showToast({
      ...
      title: "Quick Sync: pulling on both machines...",
    });
    const expanded = expandPath(localPath);
    let ok = true;
    try {
      execSync(`git -C "${expanded}" pull`, {
        encoding: "utf8",
        timeout: 15000,
      });
    } catch {
      ok = false;
      showToast({ style: Toast.Style.Failure, title: `Local pull failed` });
    }
    try {
      execSync(
        `ssh -o ConnectTimeout=5 -o BatchMode=yes ${OTHER_MACHINE} "cd ${remotePath} && git pull"`,
        { encoding: "utf8", timeout: 15000 },
      );
    } catch {
      ok = false;
      showToast({
        style: Toast.Style.Failure,
        title: `${OTHER_MACHINE} pull failed`,
      });
    }
    if (ok)
      showToast({
        style: Toast.Style.Success,
        title: "Both machines pulled successfully",
      });
    loadData();
  }
  ...
  function openInTerminal(path: string) {
    try {
      execSync(`open -a Terminal "${expandPath(path)}"`);
    } catch {
      /* ignore */
    }
  }
  ...
  function sshToRemote() {
    try {
      execSync(
        `osascript -e 'tell application "Terminal" to activate' -e 'tell application "Terminal" to do script "ssh ${OTHER_MACHINE}"'`,
      );
    } catch {
      /* ignore */
    }
  ...
  return (
    <List
      isLoading={isLoading}
      navigationTitle={`Machine Sync — ${hostname}`}
      isShowingDetail
    >
      {/* Sync Comparison */}
            ...
            return (
              <List.Item
                ...
                accessories={[
                  {
                    tag:
                      remoteReachable === false ? "mxb offline" : "checking...",
                  },
                ]}
                detail={
                  <List.Item.Detail
                    markdown={detailMarkdown(local, undefined, hostname)}
                  />
                }
                actions={
                  <ActionPanel>
          ...
          return (
            <List.Item
              ...
              accessories={[
                {
                  tag: {
                    value: `${hostname}: ${local.branch}`,
                    color: local.isDirty ? Color.Orange : Color.Green,
                  },
                },
                {
                  tag: {
                    value: `${OTHER_MACHINE}: ${remote.branch || "?"}`,
                    color: remote.isDirty ? Color.Orange : Color.Green,
                  },
                },
              ]}
              detail={
                <List.Item.Detail
                  markdown={detailMarkdown(local, remote, hostname)}
                />
              }
              actions={
                <ActionPanel>
                  ...
                  />
                  <Action
                    title="Quick Sync (pull Both)"
                    icon={Icon.Download}
                    shortcut={{ modifiers: ["cmd"], key: "s" }}
                    onAction={() => quickSync(local.path, remote.path)}
                  />
                  <Action
                    title={`Push (${hostname})`}
                    icon={Icon.ArrowUp}
                    onAction={() => handleGitAction(local.path, "push")}
                  />
                  <Action
                    title={`Pull (${hostname})`}
                    icon={Icon.ArrowDown}
                    onAction={() => handleGitAction(local.path, "pull")}
                  />
                  <Action
                    title={`Push (${OTHER_MACHINE})`}
                    icon={Icon.ArrowUp}
                    onAction={() => handleRemoteGitAction(remote.path, "push")}
                  />
                  <Action
                    title={`Pull (${OTHER_MACHINE})`}
                    icon={Icon.ArrowDown}
                    onAction={() => handleRemoteGitAction(remote.path, "pull")}
                  />
                  <Action
                    title="Open in Terminal"
                    icon={Icon.Terminal}
                    onAction={() => openInTerminal(local.path)}
                  />
                  <Action
                    title={`SSH to ${OTHER_MACHINE}`}
                    icon={Icon.Terminal}
                    onAction={sshToRemote}
                  />
                </ActionPanel>
              }
            />
              ...
              detail={
                <List.Item.Detail
                  markdown={detailMarkdown(
                    repo,
                    remoteRepos.find((r) => r.path === repo.path),
                    hostname,
                  )}
                />
              }
              actions={
                <ActionPanel>
                  ...
                  <Action
                    title="Git Push"
                    icon={Icon.ArrowUp}
                    onAction={() => handleGitAction(repo.path, "push")}
                  />
                  <Action
                    title="Git Pull"
                    icon={Icon.ArrowDown}
                    onAction={() => handleGitAction(repo.path, "pull")}
                  />
                  <Action
                    title="Open in Terminal"
                    icon={Icon.Terminal}
            ...
            <List.Item
              ...
              detail={
                <List.Item.Detail
                  markdown={detailMarkdown(
                    repo,
                    localRepos.find((r) => r.path === repo.path),
                    OTHER_MACHINE,
                  )}
                />
              }
              actions={
                <ActionPanel>
                  ...
                  <Action
                    title={`Push (${OTHER_MACHINE})`}
                    icon={Icon.ArrowUp}
                    onAction={() => handleRemoteGitAction(repo.path, "push")}
                  />
                  <Action
                    title={`Pull (${OTHER_MACHINE})`}
                    icon={Icon.ArrowDown}
                    onAction={() => handleRemoteGitAction(repo.path, "pull")}
                  />
                  <Action
                    title={`SSH to ${OTHER_MACHINE}`}
                    icon={Icon.Terminal}
